...
int main(){
	...
	TFile FIL("TestHist61.root","RECREATE");
	...
	for(int i = 0;i < VSN_CHANNEL_COUNT;i++){
		for(int j = 0;j < 16;j++){
			...
			ext_histd[i][j] = new TH1S(cstrC, cstrN, 1000, 0.0, 4000.0);
	...
	while(!feof(cur_dat)){
		...
		if(iSwap){
		 ...
		 if(pBH.RecordType == 3){
			...
			std::cout << "Erronous Record Byte Number Is "<< pBH.RecordLength << std::endl;
			std::cout << "Erronous File Pointer At " << ftell(cur_dat) << std::endl;
			std::cout << "Errornous Rotation Count is " << rot_count << std::endl;
		for(int i = 0;i < VSN_CHANNEL_COUNT;i++){
			for(int j = 0;j < 16;j++){
			...
			for(int j = 2;j < ex_count - 1;j++){
				...
				if(in_chan){
					...
					if(cur_vsn < 8){
						...
						ext_histd[cur_vsn-1][chan_flag]->Fill(chan_data);
						chan_loc -= 1;
					...
					//std::cout << "Reading channel " << chan_loc << " of vsn " << cur_vsn - 1 << std::endl;
						...
						ext_histd[cur_vsn-2][chan_flag]->Fill(chan_data);
						chan_loc -= 1;
	...
	std::cout << total_buf_read  << " + Events: " << total_ev_read << std::endl;
